...
public class BookController {
    ...
    public String getEditBookPage(@RequestParam("id") String bookId, Model model) {
        ...
        optionalBook.ifPresent(book -> {
            ...
            model.addAttribute("bookCommentsList", cs.findAllCommentsByBookId(bookId));
            model.addAttribute("isPublicBook", aclService.isPublicBookState(bookId));
            model.addAttribute(book);
        });
    ...
    @PostMapping("/books/edit/{id}/acl/{isPublic}")
    public String toggleBookAccess(@PathVariable("id") String bookId, @PathVariable("isPublic") boolean isPublic) {
        aclService.initBookPermissions(bookId, isPublic, true);
        return "redirect:/books/edit?id=" + bookId;
    }
}
